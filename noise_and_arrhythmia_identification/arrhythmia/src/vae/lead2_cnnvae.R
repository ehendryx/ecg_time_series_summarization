...
intermediate_dim2 <- 20
intermediate_dim3 <- 30
intermediate_dim4 <- 40
intermediate_dim5 <- 50
latent_dim <- 50
original_dim <- c(150, 1)
...
decoder_outputs <- latent_inputs %>%
  ...
  layer_reshape(target_shape = c(50, 1)) %>%
  # Hidden layer 1
  layer_conv_1d_transpose(intermediate_dim5, 3, padding = "same") %>%
  layer_batch_normalization() %>%
  layer_activation_relu() %>%
  # Hidden layer 2
  ...
  layer_activation_relu() %>%
  # Hidden layer 5
  layer_conv_1d_transpose(intermediate_dim1, 3, padding = "same") %>%
  layer_batch_normalization() %>%
  layer_activation_relu() %>%
  layer_flatten() %>%
  layer_dense(150, activation = "linear")
...
# Train function
vae %>% fit(ds1.train.x[,,2], epochs = 50, batch_size = 128)
# Save the final weights of the trained model to a "weights" folder
vae %>% save_model_weights_tf("weights/lead2_cnnvae")



# Predicting with the VAE  ------------------------------------------------------
...
load_model_weights_tf(vae, "D:/Documents/UCO/Research/CNN-VAE Weights/Lead2 CNNVAE Weights/lead2_cnnvae")



# Use the loaded VAE to encode the current lead's data
x_test_encoded <- predict(vae$encoder, ds1.test.x[,,2], batch_size = 200)
eps <- rnorm(dim(ds1.test.x[,,2])[1])
...
x_train_encoded <- predict(vae$encoder, ds2.train.x[,,2], batch_size = 200)
# Encoding and sampling from DS2 Test set
x_test_encoded <- predict(vae$encoder, ds2.test.x[,,2], batch_size = 200)
...
save(ds2.train.latent, ds2.test.latent, file = "lead2_1D_100mv_5layer.RData")
...
mse = mean((x_test_decoded - ds1.test.x[,,2])^2)



# Using lead 2 data
plot(1:150, ds1.test.x[500,,2], type = "l", col = "red", xlab = "Sample Number", ylab = "mV") # True beat in red
lines(1:150, x_test_decoded[500,], col = "blue") # VAE beat in blue
