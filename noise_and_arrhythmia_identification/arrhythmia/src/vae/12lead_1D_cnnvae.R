...
load("D:/Documents/UCO/Research/Data Sets/INCART12/combined_data_fixed.RData")


intermediate_dim1 <- 12 # Original 12
intermediate_dim2 <- 24 # Original 24
intermediate_dim3 <- 48 # Original 48
intermediate_dim4 <- 96 # Original 96
intermediate_dim5 <- 192 # Original 192

latent_dim <- 600
original_dim <- c(150, 12)
...
decoder_outputs <- latent_inputs %>%
  ...
  # Use c(25, 24) for latent dim size of 600
  # Use c(25, 2) for latent dim of size 50
  layer_reshape(target_shape = c(25, 24)) %>%
  # Hidden layer 1
...
vae %>% save_model_weights_tf("weights/12lead_1D_cnnvae")
...
load_model_weights_tf(vae, "D:/Documents/UCO/Research/CNN-VAE Weights/12lead_1D_600mv_5layer Weights/12lead_1D_cnnvae")
...
x_train_encoded <- predict(vae$encoder, ds2.train.x, batch_size = 200)
# Encoding and sampling from DS2 Test set
x_test_encoded <- predict(vae$encoder, ds2.test.x, batch_size = 200)
...
save(ds2.train.latent, ds2.test.latent, file = "12lead_1D_50mv_5layer.RData")
...
# Using lead 2 data
plot(1:150, ds1.test.x[600,,6], type = "l", col = "red", xlab = "Sample Number", ylab = "mV") # True beat in red
lines(1:150, x_test_decoded[600,,6], col = "blue") # VAE beat in blue
